<template>
  <div class="relative">
    ...
    <div class="relative w-80">
      <input
        type="text"
        placeholder="Search Blog (Press  &quot;/&quot; to focus)"
    ...
    <transition name="fade">
      <div v-if="query.length > 0 && searchResultsVisible" class="normal-case absolute border left-0 right-0 w-108 text-left mb-4 mt-2 rounded-lg shadow overflow-hidden z-10 overflow-y-auto" style="max-height: 32rem">
        <div class="flex flex-col" ref="results">
          ...
          >
            {{ post.item.node.title }}

            <span class="block font-normal text-copy-primary text-sm my-1">{{ post.item.node.summary }}</span>
          </a>
...
export default {
  ...
  methods: {
    ...
    performSearch() {
      this.$search(this.query, this.$static.posts.edges, this.options).then(results => {
